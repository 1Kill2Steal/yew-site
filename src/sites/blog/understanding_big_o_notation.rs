...
pub fn understanding_big_o_notation() -> Html {
    ...
    // Contents seperate HTML contents
    ...
    let dictionary_html = html! {
        <>
            ...
            <p>
                ...
                {" - This isn't measured with Big O notation. The compile time is the time
                where your interpreter/compiler to translates your written code into
                byte-code/machine code respectively."}
            </p>
            <p>
                <i>
        ...
        <>
            ...
            <ul>
                <li>
                    ...
                    {"Not a lot of algorithms are done in this complexity. Usually it's a simple
                    method on a specific data structure."}
                </li>
                <li>
                ...
                <li>
                    ...
                    {"This is pretty common to see in LeetCode. The concept of it is doing at
                    least one full iteration on your data set. Here's the tricky part though:
                    while removing the last item of a vector is "}<b>{"O(1)"}</b>{". Removing
                    its first item is actually "}<b>{"O(n)"}</b>{". You need to do a full
                    traversal from the last item to the first one in order to rearrage all the
                    items properly. Similarly, removing the last item of a singly linked list
                    is also an "}<b>{"O(n)"}</b>{" operation. You want to be careful with some
                    of the data structures because a mistake like that will easily give insane
                    runtime overhead, turning a viable solution into a bad one."}
                </li>
                <li>
                ...
                <li>
                    ...
                    Some problems have "}<b>{"O(n"}<sup>{"2"}</sup>{")"}</b>{" as the best
                    solutions. Those problems are usually quite rare (At least from my little
                    experience). A more interesting way is matrices. Those big two-dimensional
                    arrays you see in University math classes. They can be coded into a computer
                    too. The syntax is usually arr[i][j] but that's not very important right now.
                    The more important part is that these matrices are traversed in
            ...
            <ol>
                <li>
                    ...
                    <p>
                        ...
                        because it requires you to first: evaluate the square root of every
                        number, second: do modulo division for all the numbers that are less
                        than the square root of that number. How does that even work? This
                        image from the video will help understand just that:"}
                    </p>
                    ...
                    <p>
                        ...
                        get checked is whether 11 is divisible by 2 or 3 (because 4, 6, 8,
                        10 are already divisible by 2 and 6 and 9 are already divisible by 3 so the
                        math for them would be excessive). Or if you prefer the mathematical
                        explanation: "}
                    </p>
                    <p>
                        <i>
                            {"if n is divisible by some number p, then n = p × q and if q were
                            smaller than p, n would have been detected earlier as being divisible
                            by q or by a prime factor of q."}
                        </i>
                        {" ("}
                ...
                <li>
                    ...
                    <p>
                        ...
                        {", then we go to 3, we check the smallest multiple in the pool (4) and
                        if it's larger than our current number (3) we add 3 and its multiple 9
                        as a tuple of (3, 9) to the pool. The next number is 4, we check the
                        smallest multiple in the pool again, if they're equal then "}
                        <b>
                            {"increment the smallest multiple by its associated number"}
                        </b>
                        {". This means that the (2, 4) pair will become (2, 6). Then we go to
                        5, it's smaller than the smallest multiple so we add it with its
                        multiple in the pool."}
                        <br />
    ...
    let the_misinformation_about_big_o_complexity_html = html! {
        <>
            ...
            <p>
                ...
                is more efficient than algorithm B. There's more to it. The reason is this: Big
                O notation, as described in the start, is the behavior of a value (n) as it
                reaches infinity (or in mathematical notation that'd be: n → ∞). This specific
                notation eliminates all constant values in its determination. This means that
                O(2n) is portrayed as O(n) (linear), just in the same way that O(1,000n) is
                portrayed as O(n), so is O(1,000,000n) and so on. The reason for this is
                because big O notation was meant to only account for the worst case scenario of
                an algorithm. That worst case scenario being the upper limit of n as it reaches
                infinity, and because it's only looking at that, it removes all other constants
                because eventually n will be greater than them. But is this true in practice?
                In most cases it's not, some of the times you even know your exact data set as
                well! That means that you can optimize an algorithm based exactly on that data
                set."}
            </p>
        </>
    };
    ...
    wrap_blog_subsite(
        ...
        vec![
            ...
            dictionary,
            analysis,
            optimizing_for_memory_vs_space,
            the_misinformation_about_big_o_complexity,
            not_only_big_o,
            summary,
        ],
        html! {
            <>
                ...
                {optimizing_for_memory_vs_space_html}

                {the_misinformation_about_big_o_complexity_html}
